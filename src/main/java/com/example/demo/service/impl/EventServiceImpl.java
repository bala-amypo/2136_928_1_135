...
// import com.example.demo.exception.BadRequestException; // Added for Test 187
// import com.example.demo.exception.ResourceNotFoundException;
// import com.example.demo.repository.EventRepository;
// import com.example.demo.repository.UserRepository;
// import com.example.demo.service.EventService;
...
//     @Override
//     public Event createEvent(Event event) {
//         // Fix for 500 error & Test 187: Validate Publisher existence AND Role
//         if (event.getPublisher() != null && event.getPublisher().getId() != null) {
//             User publisher = userRepository.findById(event.getPublisher().getId())
//                     .orElseThrow(() -> new ResourceNotFoundException("Publisher not found"));
            
//             // ✅ TEST 187 FIX: Check if the user is allowed to create events
//             // Typically, SUBSCRIBERs are prohibited from creating events.
//             if (publisher.getRole() == UserRole.SUBSCRIBER) {
//                 throw new BadRequestException("Subscribers are not authorized to create events");
//             }
...
import com.example.demo.entity.Event;
import com.example.demo.entity.User;
import com.example.demo.entity.Role; // Using your Role enum
import com.example.demo.exception.ResourceNotFoundException;
import com.example.demo.exception.BadRequestException; // For Role validation
import com.example.demo.repository.EventRepository;
import com.example.demo.repository.UserRepository;
import com.example.demo.service.EventService;
import org.springframework.stereotype.Service;
...
public class EventServiceImpl implements EventService {
    ...
    @Override
    public Event createEvent(Event event) {
        // 1. Validate Publisher and Role (Fixes Test Case 187 & 500 Errors)
        if (event.getPublisher() != null && event.getPublisher().getId() != null) {
            User publisher = userRepository.findById(event.getPublisher().getId())
                    .orElseThrow(() -> new ResourceNotFoundException("Publisher not found with id: " + event.getPublisher().getId()));
            
            // CHECK ROLE: Test case expects BadRequestException for SUBSCRIBERs
        ...
        // 2. Defensive Default Values (Fixes MariaDB 500 Errors)
        if (event.getIsActive() == null) {
            event.setIsActive(true); // Ensure isActive is not null for DB
        }
        
        // Use standard setActive if your entity uses the 'active' boolean naming
        event.setActive(true); 
    ...
    public void deactivateEvent(Long id) {
        Event event = getById(id);
        event.setActive(false);
        // Ensure the field is consistent with your Repository query
        event.setIsActive(false); 
        eventRepository.save(event);
    }
}